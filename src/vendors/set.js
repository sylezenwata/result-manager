...
const E = {
	 ...
	 * @returns string
	 */
	data(dataName, dataValue = null) {
		let matchDataName;
		try {
			let { groups: { name } } = /^(data)?(-)?(?<name>.+)$/.exec(dataName);
			matchDataName = name;
		} catch (err) {
			if (err.name.toLowerCase() === 'typeerror') {
				throw new Error('You did not pass any data name');
			}
			throw err;
		}
		dataName = matchDataName.split('-').map((e, i) => i > 0 ? e.slice(0,1).toUpperCase()+e.slice(1) : e).join('');
		return dataValue ? (this.dataset[dataName] = dataValue) : this.dataset[dataName];
	}
};
/**
